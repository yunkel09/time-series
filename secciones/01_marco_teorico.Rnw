...
\begin{shaded}
Debido a que el p-valor es menor al nivel de significancia, decimos que la serie \textbf{no
es estacionaria} para ninguno de los dos materiales. El p-valor de la prueba es
estadísticamente significativo ($p < .001$), por lo tanto, esto respalda lo que hemos visto
en el gráfico ACF de la figura \ref{fig:corre} anterior, la serie temporal muestra un patrón
...
exponencial.} La resstricción es que todos los valores deben ser positivos.
...
por_mes |> 
  ...
  mutate(
    ...
    diferencia = difference(log_cantidad, 12)) |> 
  pivot_longer(cols = cantidad:diferencia, 
               names_to = "tipo",
               values_to = "cantidad_d") |> 
  as_tibble() |> 
  mutate(tipo = fct_inorder(tipo)) |>    
  ggplot(aes(x = fecha, y = cantidad_d)) +
  geom_line() +
